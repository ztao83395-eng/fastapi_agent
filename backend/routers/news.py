...
async def get_categories(skip: int = 0, limit: int = 100,db:AsyncSession=Depends(get_database),use_cache: bool = Query(True, description="是否使用缓存")):
    ...
    categories=await news.get_categories(db,skip,limit,use_cache)
    return{
        "code":200,
        "msg":"获取分类成功",
        "data":categories
...
async def get_news_list(
    ...
    news_list=await news.get_news_list(db,category_id,offset,page_size,use_cache)
...
async def get_news_detail(
    ...
    news_detail = await news.get_news_detail(db, news_id, use_cache=use_cache)
