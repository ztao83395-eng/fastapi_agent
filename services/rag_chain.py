...
# 导入你的缓存函数
from config.rag_cache_conf import get_json_cache, set_cache
...
class NewsRAG:
    ...
    async def ask(self, question: str, force_refresh: bool = False) -> dict:
        # 1. 检查缓存
        ...
        result = await loop.run_in_executor(None, self.qa_chain.invoke, question)
        ...
        if self.enable_cache:
            await set_cache(cache_key, response, expire=3600)
