...
用法（在项目根目录）：
  python -m scripts.eval.run_eval                        # 跑全部评测
  python -m scripts.eval.run_eval --rag --limit 5        # 只跑 RAG，前 5 条
  python -m scripts.eval.run_eval --agent                # 只跑 Agent
  python -m scripts.eval.run_eval --generate 10          # 从新闻库自动生成评测集
  python -m scripts.eval.run_eval --output docs/eval_report.md --judge-model qwen-plus

依赖：DEEPSEEK_API_KEY（对话/判定）+ EMBEDDING_API_KEY 或 BAILIAN_API_KEY（RAG 向量检索）、MySQL、向量库已构建（scripts/build_vectors.py）
"""
import argparse
import asyncio
import logging
