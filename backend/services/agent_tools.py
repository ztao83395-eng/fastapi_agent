...
async def search_news_by_keyword(keyword: str) -> str:
    """根据关键词搜索新闻标题和内容，返回新闻列表（最多5条，每条含标题和内容摘要）"""
    async for db in get_database():
        try:
            ...
            if not news_list:
                return "未找到相关新闻"
            # 返回标题 + 内容摘要（截取前 250 字，压平换行），
            # 让 AI 拿到正文信息后可以直接总结，而不只是看到标题
            items = []
            for n in news_list:
                summary = " ".join((n.content or "").split())[:250]
                items.append(f"ID:{n.id} 标题:{n.title}\n内容:{summary}")
            return "\n\n".join(items)
        finally:
            await db.close()
        break
...
def create_agent_executor():
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "你是一个智能新闻助手，名字叫小闻。你可以搜索新闻、收藏新闻、查看收藏列表、推荐新闻。回答要简洁有帮助。"
         "当提到或推荐任何新闻时，必须用 Markdown 链接格式输出：[新闻标题](/news/新闻ID)，"
         "新闻ID来自工具返回结果中编号。例如：[2023年我国GDP同比增长5.2%](/news/4)。不要输出没有链接的纯文本新闻标题。"
         "当用户要求总结或了解某条新闻的具体内容时，根据工具返回的「内容」字段直接总结要点，"
         "不要只说标题、不要反问用户“要不要收藏”，直接给出内容总结。"),
